...
$(document).ready(function () {
  const carousel = $(".iframe-carousel");
  ...
  // Function to set the carousel container's height
  function setCarouselHeight() {
    const maxHeight = Math.max.apply(
      null,
      carousel.find(".img-carousel").map(function () {
        return $(this).height();
      })
    );
  ...
  function showSlide(index) {
    const slides = carousel.find(".img-carousel");
    slides.hide();
    slides.eq(index).show();
  }

  function moveToNextSlide() {
    currentIndex = (currentIndex + 1) % carousel.find(".img-carousel").length;
    showSlide(currentIndex);
  }

  function startAutoSlide() {
    autoSlideInterval = setInterval(moveToNextSlide, 5000); // 5000 milliseconds = 5 seconds
  }

  // Set the initial carousel height
  setCarouselHeight();

  // Update carousel height whenever the window is resized
